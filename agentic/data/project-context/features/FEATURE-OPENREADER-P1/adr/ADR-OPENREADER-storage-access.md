# ADR-OPENREADER-storage-access: Android storage-access strategy for local document discovery
...
3. Record, in the Play Store Permissions Declaration form language (an artifact for whoever manages the store listing, not code), that OpenReader' core function is local document browsing/management, which is one of Google's own listed acceptable "All files access" use cases (file manager / on-device document management) — the store submission and its justification text are out of scope for this repository and are not authored here.
...
## Open question (resolved 2026-09-19, tracked as ODF-P5-02 in FEATURE-OPENREADER-P5)
**Which distribution channel(s) is OpenReader targeting, and on what timeline?** This is a business/product decision, not an architectural one, and it is not invented here. It directly determines whether the Play Store Permissions Declaration Form applies at all:
...
**Decision: Play Store, general release, using the special-access declaration path** (the user confirmed this directly, 2026-09-19, resolving `FEATURE-OPENREADER-P5/SRS.md` Unresolved Question 2 / ODF-P5-02). Consequences:
- `ARCHITECTURE.md` Risk 1 and this ADR's Risk 3 stay open, not closed by this decision — Google's Permissions Declaration Form review is manual and its outcome is still not guaranteed by any code or copy change.
- Mitigation 1 (the in-app disclosure screen, `TASK-007.md`) and mitigation 3 (Permissions Declaration Form language citing the file-manager/on-device document management use case) are load-bearing for this channel and must exist before submission — this ADR does not verify either exists yet.
- If Google's review becomes a recurring blocker, Alternative 3 (SAF migration) is the durable fallback already scoped above, not a new decision.
...
- `agentic/data/project-context/features/OpenReader_BRD_v1.0.md` §7.1 (Local Document Discovery), §9.16 (File Information corner cases)
