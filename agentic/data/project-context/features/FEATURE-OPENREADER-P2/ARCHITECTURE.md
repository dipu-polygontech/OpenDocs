# ARCH-OPENREADER-P2: PDF Reader (BRD Phase 2)
...
## ADR Recommendation
**Accepted**: [ADR-OPENREADER-pdf-library](adr/ADR-OPENREADER-pdf-library.md) records `pdfrx: 2.4.8` as the choice and was approved by the user on 2026-09-18 ("Approve it, start implementation"). The ADR's value is the same as `ADR-OPENREADER-storage-access`'s: a durable record of the alternatives ruled out and why (Syncfusion's licensing gate, `pdfx`'s web-only password support, `flutter_pdfview`'s missing search, `pdfrx` latest's Flutter SDK floor), so a future contributor doesn't have to redo this research.

## References
- `SRS.md` (this feature)
- `agentic/data/project-context/features/FEATURE-OPENREADER-P1/ARCHITECTURE.md`, `TECH-SPEC.md` (patterns reused)
- `agentic/data/project-context/features/FEATURE-OPENREADER-P1/adr/ADR-OPENREADER-storage-access.md` (ADR process precedent)
