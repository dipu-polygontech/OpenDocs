# ADR-OPENREADER-pdf-library: PDF rendering library for the Phase 2 reader
...
Thumbnails and jump-to-page are **not** library-provided widgets in any of the evaluated options and will be built as first-party OpenReader UI on top of `pdfrx`'s page-image/page-controller primitives — this is true regardless of which library had been chosen, so it is not a cost specific to this decision.
...
- `agentic/data/project-context/features/FEATURE-OPENREADER-P1/adr/ADR-OPENREADER-storage-access.md` (process precedent this ADR follows)
