# SRS-OPENREADER-P4: Text/CSV Readers and Platform Integration (BRD Phase 4)
...
| ODF-020 | Read CSV as grid | NOT DONE | P1. Reuses `excel_plus`'s own CSV parser (`Excel.fromCsv`, already a dependency since `FEATURE-OPENREADER-P3`) rather than adding a new CSV library or hand-rolling one — see `ARCHITECTURE.md` |
| ODF-008 | Restore reading position (TXT/CSV) | PARTIAL → this phase completes it for these formats | Same generic `reading_position` JSON column and `RecentRepository.getPosition()`/`markOpened()` reused from `FEATURE-OPENREADER-P2/TASK-010`, no migration |
...
- BRD §15 offline mandate: trivially satisfied — no library involved for TXT, and `excel_plus`'s CSV parsing is already local/offline (verified in `FEATURE-OPENREADER-P3`).
...
- CSV search: same shape as Excel's cell search (`FEATURE-OPENREADER-P3/TASK-011`), reused directly since it operates on the same parsed-rows model.
...
## References
- `agentic/data/project-context/features/OpenReader_BRD_v1.0.md` §9.14, §9.15, §10, §12, §13, §14, §15, §16, §28 (ODF-019/020), §30 (Phase 4)
- `agentic/data/project-context/features/FEATURE-OPENREADER-P3/SRS.md`, `ARCHITECTURE.md`, `tasks/TASK-011.md` (process and reuse pattern this phase follows, including the Excel grid this phase reuses)
- `agentic/data/project-context/features/FEATURE-OPENREADER-P1/tasks/TASK-009.md` (Open From Other Apps - tracked separately, referenced not duplicated)
