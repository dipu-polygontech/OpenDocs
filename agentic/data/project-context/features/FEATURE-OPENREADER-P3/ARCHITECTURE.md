# ARCH-OPENREADER-P3: Office Readers (BRD Phase 3)
...
Identified before proposing anything new, confirmed against `document_category.dart` and `FEATURE-OPENREADER-P2`'s actual implementation:
...
- `DocumentInteractionController.openDocument()` gains three more branches (`word`, `excel`, `powerpoint`), following the exact pattern the `pdf` branch established in `FEATURE-OPENREADER-P2/TASK-010` — including that branch's own lesson: the reader owns its `markOpened()` calls, `openDocument()` must not call it for a category with a real reader, since a bare call resets the position map.
...
- The "library gives primitives, OpenReader builds the widget" pattern from P2's PDF thumbnails (`PdfDocumentViewBuilder`/`PdfPageView`) is the template for the Excel grid UI below, where no library ships a ready-made spreadsheet widget.
...
- **`ExcelReaderView`**: sheet-tab bar + a custom scrollable grid widget (two-axis scroll, optional frozen header row/column) built on the parsed cell model — this is first-party OpenReader UI, not a library widget, exactly as thumbnails were for PDF.
...
- **Word**: `docx_file_viewer` — see `adr/ADR-OPENREADER-office-libraries.md`, **Accepted** 2026-09-18 and implemented the same day (`tasks/TASK-011.md`).
...
## References
- `SRS.md` (this feature)
- `FEATURE-OPENREADER-P2/ARCHITECTURE.md`, `FEATURE-OPENREADER-P2/tasks/TASK-010.md` (pattern and reuse precedent)
