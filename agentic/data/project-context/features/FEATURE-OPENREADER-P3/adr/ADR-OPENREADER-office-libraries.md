# ADR-OPENREADER-office-libraries: Word and Excel reading libraries for the Phase 3 readers
...
No paid/licensed-tier dependency (à la Syncfusion) was found necessary or preferable for Word or Excel this time — every viable candidate is MIT or Apache-2.0, so this ADR does not carry the same "product owner ruled out payment" framing `ADR-OPENREADER-pdf-library` did. It simply didn't come up: nothing worth choosing was paid.
...
- `agentic/data/project-context/features/FEATURE-OPENREADER-P2/adr/ADR-OPENREADER-pdf-library.md` (process precedent this ADR follows)
