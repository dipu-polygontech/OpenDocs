...
| ODF-P5-06 | Reconcile flagged dependency-audit items: `pdfrx` version pin (missing 2.5.0+ fixes, blocked on Flutter SDK bump), the local `file_picker` patch/override, the `receive_sharing_intent` Kotlin-version mismatch (2.4.0 required vs. project's 2.2.20), `docx_file_viewer`'s low-adoption/bus-factor risk, plus `VersionUpdateService`/`app_version_update` being entirely dead/unwired code (new finding from ODF-P5-05, `TASK-016.md`) | Dependency audit | NOT DONE, each individually documented but never revisited together | `FEATURE-OPENREADER-P2/ARCHITECTURE.md` Risk 1, `FEATURE-OPENREADER-P3/ARCHITECTURE.md` Risk 4, `FEATURE-OPENREADER-P1/tasks/TASK-009.md` delta, `TASK-016.md`, `pubspec.yaml` |
