...
| ODF-P5-05 | Audit whether any bundled dependency (`dio`, `firebase_core`, `firebase_messaging`, `internet_connection_checker_plus`) is reachable from a document-viewer code path in violation of BRD §15's offline mandate | Offline/network | DONE (2026-09-19) — satisfied, no violation found; `dio` turned out to not even be a dependency (stale SRS reference, corrected); Firebase is dormant by explicit design (`app_flavour.dart` TODO); `internet_connection_checker_plus` reachable only from dead code (`VersionUpdateService`, unwired — flagged for ODF-P5-06) | [TASK-016.md](tasks/TASK-016.md) |
| ODF-P5-06 | Reconcile flagged dependency-audit items: `pdfrx` version pin (missing 2.5.0+ fixes, blocked on Flutter SDK bump), the local `file_picker` patch/override, the `receive_sharing_intent` Kotlin-version mismatch (2.4.0 required vs. project's 2.2.20), `docx_file_viewer`'s low-adoption/bus-factor risk, plus `VersionUpdateService`/`app_version_update` being entirely dead/unwired code (new finding from ODF-P5-05, `TASK-016.md`) | Dependency audit | DONE (2026-09-19) — pdfrx pin/file_picker patch/docx_file_viewer risk reaffirmed; receive_sharing_intent's real issue (compileSdk 36 vs. 37 required) fixed, its Kotlin-version framing corrected (soft warning, not a blocker); Firebase/notifications/VersionUpdateService dead code removed (user-approved) | [TASK-017.md](tasks/TASK-017.md) |
...
| ODF-P5-08 | Accessibility pass: semantics labels for reader controls (search, zoom, page/sheet/slide navigation, favorite/share/open-with), screen-reader behavior, text scaling, contrast | Accessibility | DONE (2026-09-19), at the Question 4 baseline — `Semantics`/tooltip coverage for every interactive control across 5 readers + Home/Recent/Favorites/Settings, verified via `flutter test` semantics-tree assertions; manual TalkBack/VoiceOver device pass remains deferred (ODF-P5-09/10/11) | [TASK-018.md](tasks/TASK-018.md) |
