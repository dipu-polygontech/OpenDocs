...
## Follow-up: renamed DocumentScannerService -> FileScannerService (2026-09-19)
The user asked for this immediately after reviewing this QA pass, to leave the "document scanner" name free for a future camera-based document-scanning feature — the existing class only ever walked the filesystem for already-existing files, which is a different feature. Pure rename, no behavior change: `lib/services/utilities/document_scanner_service.dart` -> `file_scanner_service.dart`, class `DocumentScannerService` -> `FileScannerService`, plus its `.instance` static and every constructor/reference in `document_repository_impl.dart` and the two test files that touch it (`test/services/file_scanner_service_test.dart`, renamed from `document_scanner_service_test.dart`; `test/repositories/document_repositories_test.dart`'s `FixtureScanner`). `agentic/kit/config/allowed-commands.json`'s historical `dart format` argv entry (from P1's `TASK-001`) was deliberately left referencing the old filename — it's a governance log of a command that was actually run under that name at the time, not a current-state index. Verified: `flutter analyze` 0 errors/warnings (same 152 pre-existing infos), `flutter test` 122/124 (same 2 pre-existing failures, now surfaced under the new file/class name).
