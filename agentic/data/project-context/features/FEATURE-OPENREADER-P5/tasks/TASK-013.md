...
- `flutter test`: 122/124 passing. The 2 failures (`test/services/document_scanner_service_test.dart`: "discovers supported extensions..." and "includes depth eight but not depth nine") are a pre-existing bug unrelated to this task or to any change in this session — `DocumentScannerService._nameOf()` (`lib/services/utilities/document_scanner_service.dart:95-98`) splits on `/` only, so on this Windows dev host (backslash paths) it returns the full path instead of the basename. This is invisible on the CI runner itself (`ubuntu-latest`, forward-slash paths) and in real Android use (always forward-slash), so it does not affect CI's actual pass/fail signal on this codebase — but it is a genuine latent bug if this project is ever tested on Windows or the scanner's root-candidate paths change. Not fixed here: out of ODF-P5-07's scope (CI config, not test correctness), not part of the confirmed first-slice item list, and would need its own scoped fix (e.g. `package:path`'s `basename()`) rather than a one-line patch under this task.
