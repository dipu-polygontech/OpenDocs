...
TXT/CSV readers (§9.14–9.15) remain explicitly out of scope (BRD Phase 4), as does anything from Phase 2 already shipped (`FEATURE-OPENDOCS-P2`).
...
| ODF-015 | Read XLSX offline | NOT DONE | Parsing is well covered (`excel_plus`); grid UI is custom, same "library gives primitives, OpenDocs builds the widget" pattern as P2's PDF thumbnails |
...
- `RecentRepository.getPosition()` (added in `FEATURE-OPENDOCS-P2/TASK-010`) is reused as-is — it already returns an untyped `Map<String, Object?>`, so no change needed for Word/Excel/PowerPoint to read back their own differently-shaped position map.

## Interfaces
- `DocumentInteractionController.openDocument(DocumentModel document)` gains three more category branches (`word`, `excel`, `powerpoint`), each dispatching to its own reader route, exactly like the `pdf` branch `FEATURE-OPENDOCS-P2` added. The stub message remains for whatever categories still have no reader (text/csv, until Phase 4).
...
## References
- `agentic/data/project-context/features/OpenDocs_BRD_v1.0.md` §9.11–9.13, §10, §11 (Scenarios 2, 4, 7), §12, §13, §14, §15, §16, §28 (ODF-013…018), §30 (Phase 3)
- `agentic/data/project-context/features/FEATURE-OPENDOCS-P2/SRS.md`, `ARCHITECTURE.md`, `tasks/TASK-010.md` (process and reuse pattern this phase follows)
