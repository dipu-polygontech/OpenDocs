...
| KIT-06 | Evaluations compare real outputs and fail on wrong/empty/unknown cases | NOT DONE — no eval runner exists in the current tree; unlike KIT-03/04/05, this gap was confirmed still real on 2026-09-18 |
...
Run `sh agentic/kit/scripts/validate-kit.sh` for structure, the isolated synthetic demo, and the behavioral test suite (35 tests across `test_runtime.py`, `test_adoption.py`, `test_delivery.py`, `test_production.py`, `test_hooks.py` — re-verified passing on 2026-09-18; this corrects an earlier, inaccurate note in this file that the suite had been removed). The eval harness for KIT-06 was not found in the current tree and remains genuinely NOT DONE. Device command references are checked against official Android/Flutter guidance and installed Xcode command help; actual mobile-app execution is outside this kit-only change. Hosted CI has not been run locally.
