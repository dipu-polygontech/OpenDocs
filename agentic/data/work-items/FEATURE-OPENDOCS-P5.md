# FEATURE-OPENDOCS-P5: Hardening (BRD Phase 5)
...
|---|---|---|---|---|
| ODF-030 | Remain responsive with large files | Large-file / Performance | NOT VERIFIED, standing gap since P1 | [SRS.md](../project-context/features/FEATURE-OPENDOCS-P5/SRS.md) |
...
Three items are explicitly out of this phase's reach rather than silently dropped: ODF-P5-02 (a product decision, not engineering, mirroring the still-open PowerPoint question from `FEATURE-OPENDOCS-P3`), and ODF-P5-09/10/11 (large-file corpus, performance profiling, native/device verification — all blocked on an Android SDK/emulator/CI environment that has never existed in this project). `ARCHITECTURE.md` Risks documents what a future device/CI-equipped session would need to actually close these, rather than attempting a lower-fidelity substitute that could be mistaken for real verification.

## Validation and handoff

Not yet implemented — scoping only. Awaiting confirmation of `SRS.md`'s Unresolved Questions (implementation order, accessibility baseline depth) before starting.

## Process note

Routed through the same scoping approach as `FEATURE-OPENDOCS-P2`/`P3`/`P4` — not run through the `agentic-runtime` CLI task protocol (no governed run exists for this scoping pass). `context_status` in `agentic/data/project-context/project.yaml` remains `PARTIAL`; this document does not change that.
